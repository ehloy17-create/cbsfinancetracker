...
export default function PaymentModal({
  ...
  const splitReady  = splitMode
    ...
    : (isExact || tenderedNum >= adjustedTotal);
  ...
  async function handlePost() {
    ...
    try {
      ...
      // For non-cash methods (GCash) there is no separate tendered input — amount = total due
      const amtTendered   = splitMode ? splitTotal : (method === 'cash' ? tenderedNum : adjustedTotal);
              ...
              {method === 'cash' ? (
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">
                    {method === 'charge' ? 'Charge Notes / Reference' : 'Reference / Transaction No.'}
                  </label>
                  <input
                    ref={tenderedRef}
                    type="text"
                    ...
                    placeholder={method === 'charge' ? 'Optional account note' : 'Optional'}
